...
def FSM_JUMP_UNTIL_HOME() -> callable:
    ...
    while not utils.wait_for_img(Navigations.Dock.Buttons.Exit.images, period=0):
        if utils.wait_for_img(Navigations.Icons.GoalGates, period=0):
            ...
            if not utils.wait_for_img(Navigations.Actions.MakeJump.images, period=2) or not utils.left_click(Navigations.Actions.MakeJump.images):
                continue

            while utils.wait_for_img(Navigations.Icons.GoalGates, threshold=0.99, period=1):
                continue

        if utils.wait_for_img(Navigations.Actions.Dock, period=0):
            while utils.wait_for_img(ShipControls.Speed.Inactive, period=0):
...
def FSM_FIND_ORE() -> callable:
    ...
    while not utils.wait_for_imgs((Navigations.Objects.Resources.Types.Pyroxeres.images, Navigations.Objects.Resources.Types.Scordite.images), (), period=0):
